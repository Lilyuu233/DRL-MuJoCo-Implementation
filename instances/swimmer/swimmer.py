...
def learn(algo, actor_critic, env, device, log_dir, env_name,
    ...
    def conjugate_gradient(fvp_fn, g, x, nsteps_cg=10, residual_tol=1e-10): # 改成用Nonlinear Conjugate Gradient Optimizer forDNN Training
        r = g - fvp_fn(x)
        p = r.clone()
        rdotr = torch.dot(r, r)
    ...
    tepochs = trange(epochs, desc='Epoch', leave=True) # 修改为 epochs 而不是 epochs + 1
...
def train_fn(device_id):
    ...
    # 2. (修改) 调整训练步数以匹配论文
    # 论文参数: 200 policy iterations, 50K sim steps per iter.
    policy_iterations = 200
    nsteps = 50000  # Sim. steps per iter.
    total_timesteps = policy_iterations * nsteps # = 10,000,000
    
    # 保持不变的参数
    nminibatches = 32 # 可以根据内存调整，32是一个合理的起点
    gamma = 0.99      # 与论文一致
    lam = 0.95
    noptepochs = 10
    lr = 3e-4
...
def main():
    parser = argparse.ArgumentParser(description='Train TRPO for Swimmer-v5 based on paper parameters.')
