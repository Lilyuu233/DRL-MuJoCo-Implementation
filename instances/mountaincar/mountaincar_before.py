...
def learn(algo, actor_critic, writer, env, device, log_dir, env_name,
    ...
    def conjugate_gradient(fvp_fn, g, nsteps_cg=10, residual_tol=1e-10):
        x = torch.zeros_like(g) # 每次都从零向量开始
        r = g.clone()
        p = r.clone()
        rdotr = torch.dot(r, r)
    ...
    # +++ 新增部分: 训练结束后，自动绘制奖励曲线图 +++
    if epoch_history and rewards_history:
        print("\nTraining finished. Generating plot...")
        plt.style.use('ggplot')
        plt.figure(figsize=(10, 6.5))
        
        plt.plot(epoch_history, rewards_history, label='TRPO (Original CG)')

        # 如果安装了pandas，额外绘制一条平滑的移动平均线
        if PANDAS_AVAILABLE:
            rolling_mean = pd.Series(rewards_history).rolling(window=5, min_periods=1).mean()
            plt.plot(epoch_history, rolling_mean, linestyle='--', label='5-Epoch Rolling Average')

        plt.title(f'TRPO Training on {env_name}')
...
def train_fn(device_id):
    ...
    kwargs = {k: v for k, v in locals().items() if k not in ['writer', 'env', 'actor_critic', 'kwargs', 'device_id', 'time_now', 'var_counts']}
    json.dump(kwargs, open(f"{log_dir}/kwargs.json", 'w'), default=lambda o: '<not serializable>')
    
    # +++ 修改部分: 将 env_name 传入 learn 函数 +++
    learn(algo, actor_critic, writer, env, device, log_dir, env_name,
